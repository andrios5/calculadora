...
tem.addEventListener('click', converterT)
    ...
    // Obtém os elementos do DOM relacionados à conversão de moedas
    ...
    const tax_info = document.getElementById('tax_info'); // Exibe a taxa de câmbio
    const swap = document.getElementById('swap'); // Botão para trocar as moedas selecionadas
    
    // Adiciona eventos para atualizar os cálculos ao alterar os valores ou moedas
    ...
    swap.addEventListener('click', infoSwap); // Troca as moedas ao clicar no botão
    
    main(); // Inicializa a aplicação

    // Função principal para configurar as opções de moedas e realizar o cálculo inicial
    function main() {
      let currency = { "BRL": "Real", "EUR": "Euro", "USD": "Dollar" }; // Moedas disponíveis
      let options = [];
      for (var [key, value] of Object.entries(currency)) {
        options.push(`<option value='${key}'>${value}</option>`); // Cria as opções do dropdown
      }
      label_from_currency.innerHTML = options.join('\n'); // Preenche o dropdown de origem
      label_to_currency.innerHTML = options.join('\n'); // Preenche o dropdown de destino
      calculate(); // Realiza o cálculo inicial
    }
    
    // Função para trocar as moedas selecionadas
    function infoSwap() {
      let temp = label_from_currency.value; // Armazena o valor da moeda de origem
      label_from_currency.value = label_to_currency.value; // Troca o valor da moeda de origem pela de destino
      label_to_currency.value = temp; // Troca o valor da moeda de destino pela de origem
      calculate(); // Realiza o cálculo após a troca
    }
    
    // Faz uma requisição para obter os dados da API de câmbio
    async function getURL(url) {
      return (await fetch(url)).json(); // Retorna os dados da API em formato JSON
    }
    
    // Obtém o texto da moeda selecionada no dropdown
    ...
    async function calculate() {
      let from = label_from_currency.value; // Moeda de origem
      let to = label_to_currency.value; // Moeda de destino
      let { rates } = await getURL(`https://api.exchangerate-api.com/v4/latest/${from}`); // Obtém as taxas de câmbio da API
      let rate = rates[to]; // Taxa de câmbio entre as moedas selecionadas
      tax_info.innerText = `1 ${getInfoSelect(label_from_currency)} = ${rate} ${getInfoSelect(label_to_currency)}` // Exibe a taxa de câmbio
      input_to_ammount.value = (input_from_ammount.value * rate).toFixed(2); // Calcula e exibe o valor convertido
    }